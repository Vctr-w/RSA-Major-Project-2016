...
def convertNHeading(heading):
    return math.pi - float(heading) * math.pi / 180

def process_line(line, time_difference):
    # nao heading is in degrees
    # nao heading variance is in radians^2
    split_list = line.split('|')
    nao_log = {'Time': int(float(split_list[0]) - time_difference), 'Xpos': convertNX(split_list[2]), 'Xvar': float(split_list[4])/10, \
    'Ypos': convertNY(split_list[6]), 'Yvar': float(split_list[8])/10, \
