...
if len(sys.argv) != 2:
    print("Usage: " + sys.argv[0] + " [MATCHED LOG FILE]")
    exit(1)
...
def draw_observed(frame, left, right, location, ball_centre):
    ...
    cv2.arrowedLine(frame, tuple((location[0], location[1])), tuple((Line_end[0], Line_end[1])), (255, 255, 255), 2)
...
def main():
    ...
    actual_fn = os.getcwd() + "/actual_full_field.png"
    ...
    while True:
        ...
        if values.get('Xpos') is not None:
            ...
            for i, centre in enumerate(list_of_centres):
                ...
                draw_nao_object(frame=actual_img_resize, observed_x=values['Location'][0], observed_y=values['Location'][1], \
                                observed_h=values['Heading'], distance=b_dist, heading=b_heading, label="C", colour=(0, 0, 0))

            list_of_penalties = values['Penalty']
            for i, penalty in enumerate(list_of_penalties):
                [b_dist, b_heading, b_orient] = [float(val) for val in penalty.split(",")]
                draw_nao_object(frame=actual_img_resize, observed_x=values['Location'][0], observed_y=values['Location'][1], \
                                observed_h=values['Heading'], distance=b_dist, heading=b_heading, label="P", colour=(0, 0, 0))
